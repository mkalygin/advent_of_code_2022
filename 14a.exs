...
defmodule Cave do
  ...
  def simulate(cave) do
    do_simulation(cave, depth(cave), @sand_source)
  end
  ...
  defp do_simulation(cave, abyss, {_, y}) when y >= abyss, do: cave

  defp do_simulation(cave, abyss, {x, y}) do
    cond do
      not is_map_key(cave, {x, y + 1}) ->
        do_simulation(cave, abyss, {x, y + 1})

      not is_map_key(cave, {x - 1, y + 1}) ->
        do_simulation(cave, abyss, {x - 1, y + 1})
      ...
      true ->
        cave
        |> Map.put_new({x, y}, :sand)
        |> do_simulation(abyss, @sand_source)
    end
  end
end
